...
from skelesign.constants import PAD_TOKEN
from skelesign.dtw import dtw
...
def plot_video(
    ...
    if references is None:
        ...
        video = cv2.VideoWriter(
            video_file, fourcc, float(FPS), (1300, 650), True
        )  # Long
...
def alter_DTW_timing(pred_seq, ref_seq):
    ...
    d, cost_matrix, acc_cost_matrix, path = dtw(
        ref_seq[:, :-1], pred_seq[:, :-1], dist=euclidean_norm
    )
